...
#To be used for all remote data (removes OID portion from $lldpRemPortDesc to obtain only the ports with LLDP information. This has port + device index if there is more than 1 device in the port, like Shoretel + PC)
...
while ($index -lt $lldpInfo.Count) {
	...
	$SystemIP = Invoke-SNMPv3Walk -OID "$ifIP.$($portNumbers[$index]).1.4" @Request				#OID works as OID.[port].[indexIfMoreThan1Device].1.4
